/****************************************************************************************************************
 * @file     pax.h
 * @author   Randy J. Spaulding <randys006@yahoo.com>
 * @version  0.1
 ***********************************************************************************************************/
/****************************************************************************************************************
 * @mainpage
 ...
 * # Leading whitespace is allowed on a line, but should only be used for aligning columns.
 ...
 * #        - The master list is contained in the C++ library.
 * #        - Other language-specific pax libraries cannot add new types. Add C++ support first, then
 * #          add support in all official libraries.
 * #        - Pax libraries are free to decide how to map pax types to language-specific internal types.
 * #    - The raster consists of a multidimensional array of elements using a generic type system. All
 * #      information about the data is specified using a series of unique tags:
 * #        - BYTES_PER_VALUE: specifies the number of bytes in each value, e.g. 4 for 32-bit int, 8 for double, etc.
 * #        - VALUES_PER_ELEMENT: specifies the number of values in each element, e.g. 2 for a 2D (X, Y) type;
 * #          3 for a 3D angle (rho, theta, phi); etc.
 * #        - The elements are arranged into a specified number of dimensions with the length of each
 * #          listed individually, e.g. ELEMENTS_IN_SEQUENTIAL_DIMENSION, ELEMENTS_IN_STRIDED_DIMENSION, etc.
...
#undef X
        ...
        static paxTypes_e getPaxFileType(paxBufPtr bufPtr, float *version = NULL) {
            ...
            return paxType;
        } // paxTypes_e getPaxFileType (paxBufPtr bufPtr, float *version = NULL)
...
#define X(name,val,bpv,VPE) case val: { PAX_LOG (2, << "bpv for pax type " << val << " is " << bpv); _bpv = bpv; } break;
